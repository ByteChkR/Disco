...
internal static class TestCluster
{
    ...
    public static async Task Run(string[] args)
    {
        ...
        await Parallel.ForEachAsync(Enumerable.Repeat(0, 20), CancellationToken.None, async (i, token) =>
        {
        ...
        var result = await client.EnqueueAndWait(CancellationToken.None, AddTask.NAME, 0, JToken.FromObject(new AddArgs { A = 12312, B = 1231231 }),
            "disco://pool/high");
        sw.Stop();
        if (result.IsError)
        {
